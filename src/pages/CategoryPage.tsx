...
export const CategoryPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      storeService.getCategoryBySlug(slug),
      storeService.getProducts({ categorySlug: slug }),
    ]).then(([cat, prods]) => {
      setCategory(cat);
      setProducts(prods);
      setLoading(false);
  ...
  if (!category && !loading) {
    return (
      <div className="max-w-xl mx-auto py-20 px-4 text-center">
        <h2 className="font-serif text-2xl font-bold text-[#163E2B] mb-2">Categoría no encontrada</h2>
        <Link
          to="/productos"
    ...
    <div className="min-h-screen bg-[#FCFAF8] py-8 sm:py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <nav className="flex items-center gap-2 text-xs text-[#8D9B91] mb-6">
          ...
          <ChevronRight className="w-3.5 h-3.5" />
          <span className="text-[#163E2B] font-semibold">{category?.name}</span>
        </nav>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-3xl p-12 text-center border border-[#EFE9E1] max-w-md mx-auto">
            <p className="text-sm font-semibold text-[#163E2B]">No hay productos disponibles en esta categoría actualmente.</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
            {products.map((p) => (
